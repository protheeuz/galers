...
void setup() {
  ...
  Serial.println("WiFi connected");
    // Inisialisasi sensor dan konfigurasi WiFi  

    Serial.println("ESP32-CAM IP Address: " + WiFi.localIP().toString());


    server.on("/control_sensors", HTTP_GET, [](){
        // Ambil parameter dari URL (contoh: /control_sensors?action=activate)
        String action = server.arg("action");

        // Panggil fungsi untuk mengontrol sensor berdasarkan parameter
        controlSensors(action);

        // Kirim respons ke Flask
        server.send(200, "text/plain", "Sensors controlled");
    });

    // Sisipkan endpoint-endpoint lain sesuai kebutuhan
...
void loop() {
    server.handleClient();
    // Lakukan tugas-tugas lainnya pada loop
}

void controlSensors(String action) {
    // Implementasikan kontrol sensor berdasarkan parameter action
    // Misalnya, nyalakan buzzer, LED, dan vibration motor sesuai kebutuhan
...
void activateSensors() {
    // Nyalakan buzzer, LED, vibration motor, dll.
    // Misalnya, kirim sinyal ke GPIO untuk menyalakan komponen-komponen tersebut
...
void deactivateSensors() {
    // Matikan buzzer, LED, vibration motor, dll.
    // Misalnya, kirim sinyal ke GPIO untuk mematikan komponen-komponen tersebut
